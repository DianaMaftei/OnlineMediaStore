...
public class OrderService {
	...
	public void addItemToCart(ArrayList<? extends Product> productList, int[] itemToAdd) {
		// index 0 is the product index and index 1 is the quantity to remove
		if ((itemToAdd[0] > 0 && itemToAdd[0] <= productList.size())) {
			OrderLine newOrderLine = new OrderLine((productList.get(itemToAdd[0] - 1)), itemToAdd[1]);
			for (int i = 0; i < OnlineStoreMain.currentOrder.getOrderLines().size(); i++) {
				if (OnlineStoreMain.currentOrder.getOrderLines().get(i).equals(newOrderLine)) {
					...
					OnlineStoreMain.currentOrder.getOrderLines().get(i).setQuantity(
							OnlineStoreMain.currentOrder.getOrderLines().get(i).getQuantity() + itemToAdd[1]);
